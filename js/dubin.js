...
// ── Папки проекта (RAW, Hardsubs, Softsubs) ──
    ...
    return `<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px;">
        ${folders.map(f => {
            ...
            return `<div class="dubin-subfolder">
                ...
                ${link
                    ? `<a href="${esc(link)}" target="_blank" class="btn btn-sm"
                           style="background:${f.color};text-decoration:none;display:inline-flex;align-items:center;gap:5px;">
                           <i class="fas fa-external-link-alt"></i> Открыть в Drive</a>`
...
function renderVoiceFiles(files, projId, isAdmin) {
    return `<div style="margin-top:12px;">
        <p style="font-size:11px;font-weight:800;color:var(--dub);text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;">
            <i class="fas fa-microphone-alt"></i> Загруженная озвучка</p>
        ${files.map((f, i) => `
        <div class="dubin-file-row">
            <div style="display:flex;align-items:center;gap:10px;flex:1;min-width:0;">
                <i class="fas fa-${f.format === 'flac' ? 'music' : 'volume-up'}"
                   style="color:var(--dub);font-size:16px;flex-shrink:0;"></i>
                <div style="min-width:0;">
                    <div style="font-size:13px;font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
                        ${esc(f.name)}</div>
                    <div style="font-size:10px;color:var(--text-dim);">
                        ${f.format?.toUpperCase()} • ${esc(f.author)} • ${new Date(f.date).toLocaleDateString()}</div>
                    ${f.comment
                        ? `<div style="font-size:10px;color:var(--text-dim);font-style:italic;">${esc(f.comment)}</div>`
                        : ''}
                </div>
            </div>
            <div style="display:flex;gap:6px;flex-shrink:0;">
                <a href="${esc(f.link)}" target="_blank" class="btn btn-sm btn-purple"
                   style="text-decoration:none;"><i class="fas fa-download"></i></a>
                ${isAdmin
                    ? `<button class="btn-sm" style="background:#ef4444;"
                           onclick="deleteVoiceFile('${projId}',${i})">
            ...
            <div class="dubin-folder-header" onclick="toggleDubinFolder('${p.id}')">
                ...
                <div style="flex:1;">
                    ...
                    <div style="font-size:11px;color:var(--text-dim);margin-top:2px;">
                        <span class="tag" style="background:${statusColor(p.status)};">
                            ${esc(p.status || 'В работе')}</span>
                        <span class="year-tag">${esc(p.genre)}</span>
                    </div>
                    ${p.notes
                        ? `<p style="font-size:12px;color:var(--text-dim);margin-top:4px;">${esc(p.notes)}</p>`
                        : ''}
                </div>
                <div style="display:flex;align-items:center;gap:8px;">
                    ${isAdmin ? `
                        <button class="btn-sm" style="background:#3897f0;"
                            onclick="event.stopPropagation();openDubinProjectModal('${p.id}')">Ред</button>
                        <button class="btn-sm" style="background:#ef4444;"
                            onclick="event.stopPropagation();deleteDubinProject('${p.id}')">Удал</button>
                    ` : ''}
            ...
            <div class="dubin-folder-body" id="folder-${p.id}">
                ...
                <div style="margin-top:14px;padding-top:14px;border-top:1px solid var(--border);">
                    <button class="btn btn-purple btn-sm" onclick="openUploadVoice('${p.id}')">
                        <i class="fas fa-upload"></i> Загрузить озвучку (mp3/flac)
                    </button>
...
export function bindDubin(db, auth, getState) {
    ...
    // Открыть модал загрузки аудиофайла
