...
// Текущий открытый участник (нужен для редактирования страницы)
export let curTM = null;

export function bindTeam(db, getState) {

    // ── Загрузка и отрисовка команды ──
        ...
        Object.keys(cats).forEach(cat => {
            ...
            cDiv.innerHTML = `
                <div class="team-container-header"
                     onclick="const g=document.getElementById('cat-${safeId}');g.style.display=g.style.display==='none'?'grid':'none'">
                    ${esc(cat)} <i class="fas fa-chevron-down" style="font-size:11px;"></i>
                </div>
                <div class="grid" id="cat-${safeId}"></div>`;
            w.appendChild(cDiv);
                ...
                <div class="card" data-id="${m.id}" onclick="openTeamPage('${m.id}')">
                    ...
                    ${isAdmin ? `<div class="adm-tools">
                        <button class="btn-sm" style="background:#3897f0;"
                            onclick="event.stopPropagation();openTeamModal('${m.id}')">Ред</button>
                        <button class="btn-sm" style="background:#ef4444;"
                            onclick="event.stopPropagation();delTeam('${m.id}')">Удал</button>
                    </div>` : ''}
                    <img src="${esc(m.img)}" loading="lazy"
    ...
    // ── Открыть модал добавления/редактирования участника ──
    window.openTeamModal = async (id = '') => {
        ...
        if (id) {
            ...
            ['name', 'role', 'img', 'cat'].forEach(f =>
                document.getElementById(`ad-m-${f}`).value = d[f] || '');
        } else {
            ['name', 'role', 'img', 'cat'].forEach(f =>
                document.getElementById(`ad-m-${f}`).value = '');
        }
    ...
    window.saveTeam = async () => {
        ...
        if (!data.name) return showToast('Введите никнейм участника!', 'error');
    ...
    window.openTeamPage = async (id) => {
        ...
        document.getElementById('own-tp-controls').style.display =
            (isLinked && !isAdmin) ? 'flex' : 'none';

        document.getElementById('team-page-view').innerHTML = `
            <div style="display:flex;gap:28px;align-items:flex-start;flex-wrap:wrap;">
                <img src="${esc(curTM.img)}"
                     style="width:230px;height:320px;border-radius:14px;object-fit:cover;border:2px solid var(--accent);box-shadow:var(--shadow);"
                     onerror="this.src='${PLACEHOLDER_TEAM_IMG}'">
                <div style="flex:1;min-width:280px;">
                    ...
                    <div style="background:var(--input-bg);padding:18px;border-radius:10px;border:1px solid var(--border);margin-bottom:18px;">
                        ...
                        <p style="line-height:1.6;font-size:14px;white-space:pre-wrap;">
                            ${esc(curTM.bio || 'Информация пока не добавлена.')}</p>
                    </div>
                    ${curTM.social
                        ? `<a href="${esc(curTM.social)}" target="_blank" class="btn btn-outline"
                               style="text-decoration:none;"><i class="fas fa-link"></i> Соцсети</a>`
                        : ''}
                </div>
            </div>`;
    };
    ...
    window.deleteTP = async () => {
        if (!curTM || !confirm('Удалить страницу участника?')) return;
        await updateDoc(doc(db, 'team', curTM.id), { bio: '', social: '' });
        showToast('Страница удалена');
        navigate('team');
    };

    // ── Редактирование своей карточки (Dub-пользователь) ──
    window.openMyTPEdit = () => {
