    ...
    if (userData) {
        ...
        if (!viewedSnap.exists()) {
            viewTimer = setTimeout(async () => {
                ...
                await setDoc(doc(db,`users/${auth.currentUser.uid}/viewed`,id), { at: Date.now() });
...
// ── Трейлер — подготовка URL (mute + autoplay) ──
function trailerSrc(url) {
...
window.enableTrailerSound = () => {
    ...
    const src = iframe.src.replace('&mute=1','&mute=0').replace('&controls=0','&controls=1');
    iframe.src = src;
...
function renderEpGrid(series, isAdmin) {
    document.getElementById('v-ep-list').innerHTML = series.map((ep, i) => `
        <div class="ep-card ${i===currentEpIdx?'ep-card--active':''}" onclick="playEpByIdxGlobal(${i})">
            <div class="ep-card-thumb">
                ${ep.thumb ? `<img src="${esc(ep.thumb)}" onerror="this.parentNode.innerHTML='<i class=\\'fas fa-film\\'></i>'">` : '<i class="fas fa-film"></i>'}
                ${isAdmin ? `<button class="ep-card-del" onclick="event.stopPropagation();delEp(${i})">×</button>` : ''}
            </div>
    ...
    document.getElementById('fs-ep-panel').innerHTML = series.map((ep, i) => `
    ...
    window.loadMyLists = async () => {
        ...
        container.innerHTML = `
            <div class="lists-section">
                <h4 style="margin-bottom:14px;color:var(--accent);">⭐ Избранное (${favorite.length})</h4>
                ${favorite.length ? `<div class="lists-grid">${favorite.map(r=>`
                ...
                : '<p style="color:var(--text-dim);font-size:13px;">Пусто</p>'}
            </div>
            <div class="lists-section" style="margin-top:24px;">
                <h4 style="margin-bottom:14px;color:var(--text-dim);">🕐 Буду смотреть (${later.length})</h4>
                ${later.length ? `<div class="lists-grid">${later.map(r=>`
                    <div class="list-card" onclick="openView('${r.relId}')">
                        <img src="${esc(r.img)}" onerror="this.src='${PLACEHOLDER_IMG}'">
                        <div class="list-card-title">${esc(r.title)}</div>
                    </div>`).join('')}</div>`
            ...
            <div class="lists-section" style="margin-top:24px;">
                <h4 style="margin-bottom:14px;color:var(--text-dim);">👁 Просмотрено (${viewed.length})</h4>
                <p style="font-size:12px;color:var(--text-dim);margin-bottom:12px;">Релиз засчитывается после 10 мин просмотра.</p>
                ${viewed.length ? viewed.map(v=>`
                    <div style="padding:8px 0;border-bottom:1px solid var(--border);font-size:13px;color:var(--text-dim);">
                        <span style="color:white;">${v.id}</span>
                        <span style="float:right;">${new Date(v.at).toLocaleDateString()}</span>
                    </div>`).join('')
                : '<p style="color:var(--text-dim);font-size:13px;">Пусто</p>'}
            </div>`;
    };
