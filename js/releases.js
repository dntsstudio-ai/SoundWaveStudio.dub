// ============================================================
//  js/releases.js — Релизы, плеер SWS, избранное, просмотренное
// ============================================================
...
import {
    ...
    playerSeekTo, getYtVideoId, buildEmbedSrc
} from './player.js';
...
const MAIN_PLAYER_ID    = 'sws-main-player';
const TRAILER_PLAYER_ID = 'sws-trailer-player';
...
function renderViewPage(db, auth, userData, isAdmin, startEpIdx = 0) {
    ...
    document.getElementById('v-info').innerHTML = `
        <div class="view-ivi-wrap">
            ...
            ${trailer ? `
            <!-- ТРЕЙЛЕР через SWS Player -->
            <div class="trailer-section">
                <div class="trailer-label">
                    <i class="fas fa-play-circle"></i> Трейлер
                </div>
                <div class="sws-player-container" id="${TRAILER_PLAYER_ID}"></div>
            </div>` : ''}
            ...
            <div class="main-player-section">
                ...
                ${series.length === 0 ? `
                <div class="sws-player-container" id="${MAIN_PLAYER_ID}">
                ...
                <div class="sws-player-container" id="${MAIN_PLAYER_ID}"></div>`}
    ...
    // Инициализируем трейлер
    if (trailer?.url) {
        initPlayer(TRAILER_PLAYER_ID, {
            url:      trailer.url,
    ...
    // Инициализируем основной плеер
    if (series.length > 0) {
        const firstEp = series[startEpIdx];
        initPlayer(MAIN_PLAYER_ID, {
            url:   firstEp.url,
...
// ── Таймеры заставок через player.js ──
function scheduleIntroTimers(series, idx) {
    ...
    playerHideAllOverlays(MAIN_PLAYER_ID);

    if (ep.introStart && ep.introStart > 0) {
        const t1 = setTimeout(() => {
            if (currentEpIdx !== idx) return;
            if (playerSettings.autoSkip) {
                playerSeekTo(MAIN_PLAYER_ID, (ep.introStart||0) + (ep.introDuration||90));
                return;
            }
            playerShowSkip(MAIN_PLAYER_ID, () => {
                playerSeekTo(MAIN_PLAYER_ID, (ep.introStart||0) + (ep.introDuration||90));
            });
            const t2 = setTimeout(() => {
                playerHideSkip(MAIN_PLAYER_ID);
    ...
    if (ep.outroStart && ep.outroStart > 0) {
        const t3 = setTimeout(() => {
            ...
            playerShowNext(MAIN_PLAYER_ID, () => playNextEp());
        }, ep.outroStart * 1000);
        introTimers.push(t3);
    }
}

// ── Воспроизвести эпизод через SWS Player ──
function playEp(series, idx, isAdmin) {
    ...
    playerLoad(MAIN_PLAYER_ID, ep.url, ep.name + (ep.title ? ' — ' + ep.title : ''));
